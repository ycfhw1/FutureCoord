...
class MCTS:
    ...
    #在虚拟环境中评价某个节点的价值
    @abstractmethod
    def evaluate(self, sim_env: ServiceCoordination5) -> float:
        '''Heuristic evaluation function to assess node value.'''
        pass

    def select_and_expand(self, sim_env: ServiceCoordination5) -> Tuple:
    ...
    def backpropagate(self, node: ANode, rewards: List) -> None:
        ...
        for rew in rewards:
            ...
            node = node.parent
    #估计的是状态？？？？？？？？
    def rollout(self, sim_env: ServiceCoordination5) -> float:
    ...
    @abstractmethod
    def flow_forecast(self, sim_env: ServiceCoordination5, trace: List) -> List:
        '''Prepare forecast flows for simulating service coordination.'''
        pass
    #利用rollout反馈value函数
    def predict(self, env: ServiceCoordination5, process: Traffic, **kwargs: Dict):
...
class FutureCoord(MCTS):
    ...
    def evaluate(self, sim_env: ServiceCoordination5) -> float:
        ...
        if sim_env.admission['deployed']:
            state = sim_env.calculate_accept_rate()
            state_admission = sim_env.compute_state_admission()
            return state[0]*state[1]*state[2]*state[3]/(state_admission[6]*state_admission[5]*state_admission[4])-(state_admission[0]+state_admission[1]+state_admission[2]+state_admission[3])
        else:
            return 0.0
