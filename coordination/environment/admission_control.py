...
from coordination.agents2.baselines import AllCombinations
from coordination.agents2.grc import GRC
...
#admission_control的一个主要思考就是不是所有请求都够用的，是这样吧
class ServiceCoordination5(gym.Env):
    ...
    def __init__(self, net_path: str, process: Traffic, vnfs: List, services: List):
        ...
        self.action_space = spaces.MultiDiscrete([self.ACTION_DIM_1,self.ACTION_DIM_2])
    ...
    def place_single_vnf(self, action):
        ...
        if final_placement:
            try:
                ...
                _, route = nx.single_source_dijkstra(self.net, source=action, target=self.request.egress, weight=self.get_weights,cutoff=self.request.resd_lat)
    ...
    def compute_reward(self, finalized: bool, deployed: bool, req: Request) -> float:
        ...
        if deployed:
            ...
            # print("部署成功的奖励")
            # print(req.waiting_time*req.profit/(crelease+mrelease/100)*req.duration)
            # state=self.compute_state_admission()
            # return req.waiting_time*req.profit/(crelease+mrelease/100)*req.duration*req.datarate*(state[4]/(state[8]+0.01))*(state[5]/(state[9]+0.01))*(state[6]/(state[10]+0.01))*10

            state=self.compute_state_admission()
            return (10-state[4])*(10-state[5])*(1-state[6])*10
        return 0.0
    ...
    def place_request(self, request: Request) -> bool:
        ...
        global result
        self.allcombine = AllCombinations()
    ...
    #     #所有队列都空了，直接拒绝，等风来
    #     if action==5 or len(self.all_waiting_queues[action])==0:
    #         print("当前请求队列都空了，等风来吧"+str(self.time))
    #         self.valid_actions = self.valid_actions_method()
    #         self.time = self.time + self.update_interval
    #         #你选取了不合理的动作，当前队列在这个窗口都空掉了，所以会有剩余啊，你选了一个空动作，都没有清空所有队列的嘛，我就强制更新了咋地嘛？
    #         #可是不加这一行，强化学习的agent会掉进出不来，一直在里面撞墙。他好像特别喜欢这样
    #         if self.time > self.end_time + self.TIME_WINDOW_LENGTH:
    #             self.time = self.end_time + self.TIME_WINDOW_LENGTH
    #         self.release_request()
    #         return self.compute_state_admission(), -100.0, self.done, {}
    #     if len(self.all_waiting_queues[action])!=0:
    ...
    #     if len(self.vtype_bidict.mirror[self.request]) and self.request.rejected == True:
    #         del self.vtype_bidict.mirror[self.request]
    #     self.update_actions()
    ...
    def step(self, action):
        ...
        # print("当前队列状态")
        # print(self.all_waiting_queues)
        #直接拒绝的说，不需要任何判断，按到期时间确定是否放回源队列
        if action[1] == 0:
            print(action)
            #我选到了一个空队列
            if action[0]==5 or len(self.all_waiting_queues[action[0]])==0:
                print("直接拒绝且你选到了一个空队列")
                self.valid_actions = self.valid_actions_method()
                self.time = self.time + self.update_interval
                # 你选取了不合理的动作，当前队列在这个窗口都空掉了，所以会有剩余啊，你选了一个空动作，都没有清空所有队列的嘛，我就强制更新了咋地嘛？
                # 可是不加这一行，强化学习的agent会掉进出不来，一直在里面撞墙。他好像特别喜欢这样
                ...
                print("直接拒绝但你选到了一个非空队列")
                #先从这个窗口内删除请求
                self.request = self.all_waiting_queues[action[0]][0]
                del self.all_waiting_queues[action[0]][0]
                #之前拒绝过，别再拒绝了
                try:
                    if self.request.rejected == False:
                        self.info[self.request.service].requests += 1
                        print("拒绝请求初始化")
                        print(self.info[self.request.service].requests)
                except Exception as e:
                    pass
                self.request.rejected=True
                #下一个时间窗口还有部署可能（尽可能防止请求空漂走）：
                if self.check_departure(self.end_time):
                #然后再按照时间判断是否加入拒绝队列
                    print("后面还有可能去部署")
                    self.add_reject_list(self.request)
                    self.valid_actions = self.valid_actions_method()
                    ...
                    self.release_request()
                    # time.sleep(0.5)
                    return self.compute_state_admission(), -1.0, self.done, {}
                else:
                    #the last time window you should place arbitritary
                    print("后面没机会了，在这部署吧")
                    if self.routes_bidict[self.request] == [] and self.request.rejected == True:
                        self.initial_one_rejected_request(self.request)
                    if len(self.vtype_bidict.mirror[self.request]) and self.request.rejected == True:
                    ...
                    if not self.valid_routes:
                        ...
                        self.release_request()
                        # time.sleep(0.5)
                        return self.compute_state_admission(), self.compute_reward(True, False,self.request), self.done, {}
                    # whether this place is right or not currently i should ignore it
                    self.compute_state()
                    print("开始部署")
                    state_placement, reward_step, result = self.place_request(self.request)
    ...
    #         if req.service == 0 and len(self.all_waiting_queues[0]) < self.MAX_QUEUE_LENGTH:
    #             self.all_waiting_queues[0].append(req)
    #         if req.service == 1 and len(self.all_waiting_queues[1]) < self.MAX_QUEUE_LENGTH:
    #             self.all_waiting_queues[1].append(req)
    #         if req.service == 2 and len(self.all_waiting_queues[2]) < self.MAX_QUEUE_LENGTH:
    #             self.all_waiting_queues[2].append(req)
    #         if req.service == 3 and len(self.all_waiting_queues[3])  < self.MAX_QUEUE_LENGTH:
    #             self.all_waiting_queues[3].append(req)

    # TODO:decrease the reward if the request is rejected
    def sort_rejected(self):
        # in the first timestep the reject_list is []
        self.all_waiting_queues[0] += self.all_rejected_list[0]
        self.all_waiting_queues[1] += self.all_rejected_list[1]
        self.all_waiting_queues[2] += self.all_rejected_list[2]
        self.all_waiting_queues[3] += self.all_rejected_list[3]
        #if not cleared the rejected list can be add to the remaining again even the request has expired
        self.all_rejected_list=[[],[],[],[]]
    ...
    def update_time_window(self):
        ...
        print("现在开始更新时间窗口")
        print("+++++++++++++++++++++")
        print("当前时间"+str(self.time)+"上一个时间窗口的结束时间"+str(self.end_time))
        print("在更新之前的队列")
        time.sleep(2)
        print(self.all_waiting_queues)
        self.time =self.end_time+self.TIME_WINDOW_LENGTH
    ...
    def combine_request_list(self):
        ...
        print(self.all_rejected_list)
        self.sort_rejected()
        length = 0
        if self.time !=self.TIME_WINDOW_LENGTH:
            self.check_departure(self.time)
        for queue in self.all_waiting_queues:
            length += len(queue)
        print("新的时间窗口内，请求总个数"+str(length))
        self.reconfiguration()
        # this should be true,why this can't be true???????????????
        try:
            self.update_interval = self.TIME_WINDOW_LENGTH/(length)
        except Exception as e:
            print("可能快要结束了")
        print("这一轮的更新时长" + str(self.update_interval))
    ...
    def add_reject_list(self, request: Request):
        if request.departure_time >=self.start_time +2*self.TIME_WINDOW_LENGTH:
            if request.service==0:
                self.update_profit(request)
                self.all_rejected_list[request.service].append(request)
            if request.service==1:
                self.update_profit(request)
                self.all_rejected_list[request.service].append(request)
            if request.service==2:
            ...
            request.rejected_num=request.rejected_num+1
            print("拒绝请求加入拒绝队列")
            print("_________________________")
            print("离开时间" + str(self.request.departure_time))
            print("当前时间" + str(self.end_time))
            ...
            print("_______________________________")
            # time.sleep(0.5)
    #出现这个问题的原因是过时请求在队列中超时根本没有调度到，因此在更新时间窗口的时候还得判断一次
    def check_departure(self,time):
        for queues in self.all_waiting_queues:
            for req in queues:
                if req.departure_time<time:
                    del self.all_waiting_queues[self.all_waiting_queues.index(queues)][queues.index(req)]
                    print("集中判断请求是否过时")
                    print("_________________________")
                    print("离开时间" + str(self.request.departure_time))
                    print("当前时间" + str(self.end_time))
    ...
    def compute_state_admission(self):
        ...
        mean_lutil = np.mean([self.datarate[frozenset(
            {src, trg})] / self.MAX_LINKRATE for src, trg in self.net.edges])
        state_new_arrival_memory = len(self.all_waiting_queues[0]) / self.MAX_QUEUE_LENGTH
        state_new_arrival_datarate = len(self.all_waiting_queues[1]) / self.MAX_QUEUE_LENGTH
        state_new_arrival_cpu = len(self.all_waiting_queues[2]) / self.MAX_QUEUE_LENGTH
        state_new_arrival_latency = len(self.all_waiting_queues[3]) / self.MAX_QUEUE_LENGTH
        # state_reject_memory = len(self.all_rejected_list[0]) / self.MAX_QUEUE_LENGTH
        # state_reject_datarate = len(self.all_rejected_list[1]) / self.MAX_QUEUE_LENGTH
        # state_reject_cpu = len(self.all_rejected_list[2]) / self.MAX_QUEUE_LENGTH
        # state_reject_latency = len(self.all_rejected_list[3]) / self.MAX_QUEUE_LENGTH
        # state_all_memory=state_reject_memory+state_new_arrival_memory
        # state_all_cpu = state_reject_cpu + state_new_arrival_cpu
        # state_all_latency=state_reject_latency + state_new_arrival_latency
        # state_all_datarate=state_new_arrival_datarate+state_reject_datarate
        state_queue = [state_new_arrival_memory,state_new_arrival_datarate,state_new_arrival_cpu,state_new_arrival_latency]
    ...
    def valid_actions_method(self):
        ...
        if self.needed_updating_time_window():
            ...
            for queue in self.all_waiting_queues:
                if queue != []:
                    valid_actions[self.all_waiting_queues.index(queue)] = len(queue)
        if self.time>=self.TIME_HORIZON and self.check_time_window_queue()==4:
            self.done = True
            print("本轮结束，撒花")
    ...
    def needed_updating_time_window(self):
        #这跟队列开的速率和离开速率有关
        if self.check_time_window_queue()==4 and self.end_time<self.TIME_HORIZON and (self.time % self.TIME_WINDOW_LENGTH)==0.0 and self.time!=self.TIME_WINDOW_LENGTH:
            print("队列全空时，等待下一次更新")
            return True
        #队列不全空，第一个窗口不跟新，最后一个窗口不更新
        elif (self.time % self.TIME_WINDOW_LENGTH)==0.0 and self.time!=self.TIME_WINDOW_LENGTH and self.time!=self.TIME_HORIZON:
            return True
        else:
            return False
    ...
    def compute_network_util(self):
        ...
        for u, v, w in self.net.edges(data=True):
            ...
            link_util_list.append((link_util,propagation))
        print("链路利用率")
    ...
    #         self.last_memory_state = self.compute_memory_list[-1]
    ...
    #         #计算两个窗口端点
    #         self.last_entroy_state= self.entroy_state_list[-1]
    #         self.entroy_state_list.append(entroy_compute_sum)
    #         delta_entroy=entro-self.last_entroy_state
    #         compute_temperature=abs(delta_compute)/abs(delta_entroy)
    ...
    def reconfiguration(self):
        ...
        if list_cpu!=[]:
            ...
            state_placement, reward_step, result=sim_env.place_request(sim_env.request)
            #对真实环境开始重配置
            list_cpu_real = self.get_candidate_requests_cpu()
            self.request = list_cpu_real[0]
    ...
    def get_candidate_requests_cpu(self):
        ...
        sorted_cpu = sorted(enumerate(cpu_util), key=lambda x: x[1])
        idx1 = [i[0] for i in sorted_cpu]
        nums1 = [i[1] for i in sorted_cpu]
        #CPU利用率最大的节点，取他上面的1，3，5进行迁移,所有待迁移请求
        request_cpu_3=[req for req in self.vtype_bidict[(sorted_cpu[0][0],5)]]
